...
export default function Vote({ proposals }) {
  ...
  const [core] = React.useState(['0x1Fde40a4046Eda0cA0539Dd6c77ABF8933B94260'])
    ...
    <Box padding={20}>
      <Box>
        ...
        {isLoggedIn && role === 'brand' && (
          <Box marginTop={5}>
            <Button onClick={() => route.push('/vote/create')} padding={5} backgroundColor={'brand.400'}>
        ...
        <Box>
          ...
          <Box
            ...
            <Box
              ...
              <Text marginLeft={2} color={selectionType[0] ? '#ec407a' : 'white'}>
                Core
              </Text>
            </Box>
            <Box
              ...
              <Text marginLeft={2} color={selectionType[1] ? '#ec407a' : 'white'}>
                Community
              </Text>
            </Box>
            <Box
          ...
          <Box display={'flex'} flexDir={'row'} minW={'50vw'} padding={2} background={'#1A202C'}>
          ...
          <Box border={'2px solid #ec407a'} padding={5} borderBottomRadius={25}>
            {snapshots.map((data) => {
              return (
                <Card
                  core={data.author}
...
export async function getStaticProps() {
  ...
  const { data } = await client.query({
    query: gql`
      query {
        proposals(first: 50, skip: 0, where: { space_in: ["thecreative.eth"] }, orderBy: "created", orderDirection: desc) {
          id
          title
          body
          choices
